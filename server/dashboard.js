...
router.get('/fastfilling',function(req,res){
  c={tutorlocation:req.session.location,maxstudents: { $gt: 0 },tutordate:{ $gt:new Date(todaydate)  },tutormail:{$ne:req.session.mail}}
...
router.get('/mostliked',function(req,res){
  dbo.collection('continousteacher').find({tutorlocation:req.session.location,tutormail:{$ne:req.session.mail}}).sort({"likes":-1}).limit(10).toArray(function(err,result){
    if(err) throw err
    res.send(JSON.stringify(result))
  })
})
